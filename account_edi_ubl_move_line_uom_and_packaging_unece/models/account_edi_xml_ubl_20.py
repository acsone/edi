...
class AccountEdiXmlUbl_20(models.AbstractModel):
    ...
    @api.model
    @ormcache()
    def _get_uom_by_unece_code(self):
        uoms = self.env["uom.uom"].search([("unece_code", "!=", False)])
        return {u.unece_code: u.id for u in uoms}

    def _import_fill_invoice_line_uom(self, invoice_line, unit_code):
        uom = self._get_uom_by_unece_code().get(unit_code, None)
        if uom:
    ...
    def _import_fill_invoice_line_packaging(self, invoice_line, unit_code):
        product_packaging = self.env["product.packaging"].search(
            [
                ...
                ("packaging_level_id.unece_type_ids.code", "=", unit_code),
                ("packaging_level_id.unece_type_ids.code", "=", unit_code.lstrip("X")),
            ],
            limit=1,
        )
