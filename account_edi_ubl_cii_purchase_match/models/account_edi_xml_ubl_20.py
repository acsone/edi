...
class AccountEdiXmlUBL20(models.AbstractModel):
    ...
    def _match_invoice_line_to_purchase_order_line(self, invoice_line, purchase_lines):
        """
        match an invoice line with a purchase order line
        ...
        for purchase_line in purchase_lines:
            ...
            if (
                purchase_line.product_id == invoice_line.product_id
                or purchase_line.name == invoice_line.name
                or purchase_line.product_id.name == invoice_line.name
                or invoice_line.name
                in purchase_line.product_id.seller_ids.mapped("product_name")
            ):
                invoice_line._set_product(purchase_line.product_id)
