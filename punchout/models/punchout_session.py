...
class PunchoutSession(models.Model):
    ...
    def _check_punchout_request_ok(self, response):
        ...
        if not response.ok:
            ...
            self.env.user.notify_warning(
                title=_("PunchOut request error"), message=log_msg, sticky=True,
            )
            res = False
    ...
    def _store_punchout_session_response(self, backend_id, cxml_string):
        ...
        if is_valid:
            ...
            session._notify_punchout_response_received()
        else:
            session.write(
                {"state": "error", "error_message": xml_validation.get("error")}
            )
            session._notify_response_validation_error()
    ...
    def _notify_response_validation_error(self):
        self.ensure_one()
        user = self.user_id
        user.with_user(user).notify_warning(
            title=_("Unable to process the request"),
            message=_("The response received from the backend is not valid."),
            sticky=False,
        )
